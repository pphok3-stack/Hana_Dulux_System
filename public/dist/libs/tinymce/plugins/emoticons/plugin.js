...
(function () {
    ...
    const register$2 = (editor, pluginUrl) => {
        ...
        registerOption("emoticons_database_url", {
            processor: "string",
            default: `${pluginUrl}/js/${getEmojiDatabase(editor)}${
                editor.suffix
            }.js`,
        });
    ...
    const getEmojiDatabasUSDl = option("emoticons_database_url");
    const getEmojiDatabaseId = option("emoticons_database_id");
    const getAppendedEmoji = option("emoticons_append");
    const getEmojiImagUSDl = option("emoticons_images_url");
    ...
    const initDatabase = (editor, databasUSDl, databaseId) => {
        const categories = value();
        const all = value();
        const emojiImagesUrl = getEmojiImagUSDl(editor);
        const getEmoji = (lib) => {
        ...
        editor.on("init", () => {
            global.load(databaseId, databasUSDl).then(
                (emojis) => {
    ...
    var Plugin = () => {
        global$1.add("emoticons", (editor, pluginUrl) => {
            ...
            const databasUSDl = getEmojiDatabasUSDl(editor);
            const databaseId = getEmojiDatabaseId(editor);
            const database = initDatabase(editor, databasUSDl, databaseId);
