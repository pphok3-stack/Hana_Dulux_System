...
(function () {
    ...
    const getImageSize = (url) =>
        new Promise((callback) => {
            ...
            img.onerror = () => {
                    ...
                    Promise.reject(`Failed to get image dimensions for: ${url}`)
                );
            };
    ...
    const isSafeImagUSDl = (editor, src) => {
    ...
    const sanitizeImageData = (editor, data) => {
        ...
        return {
            ...data,
            src: isSafeImagUSDl(editor, src) ? src : "",
        };
    };
    ...
    const collect = (editor) => {
        ...
        const prependURL = Optional.some(getPrependUrl(editor)).filter(
            (prUSDl) => isString(prUSDl) && prUSDl.length > 0
        );
    ...
    const imageSize = (editor) => (url) => {
        if (!isSafeImagUSDl(editor, url)) {
            return Promise.resolve({
                width: "",
                height: "",
            });
        } else {
            return getImageSize(editor.documentBasUSDI.toAbsolute(url)).then(
                (dimensions) => ({
